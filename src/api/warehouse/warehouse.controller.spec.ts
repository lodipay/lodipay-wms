...
describe('WarehouseController', () => {
  ...
  it('should get warehouse inventories list', async () => {
    ...
    jest
      .spyOn(warehouseService, 'getInventories')
      .mockImplementation(filterDto => {
    ...
    expect(await controller.getInventories(query)).toStrictEqual(paginatedDto);
  });
  ...
  it('should remove warehouse inventory', async () => {
    ...
    expect(
      await controller.deleteInventory(`${warehouse.id}`, `${inventory.id}`),
    ).toBe(undefined);
  });
});
