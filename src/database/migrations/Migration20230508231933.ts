...
export class Migration20230508231933 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'alter table "location" add column "warehouse_id" int not null;',
    );
    this.addSql(
      'alter table "location" drop constraint "location_name_unique";',
    );
    this.addSql(
  ...
  async down(): Promise<void> {
    this.addSql(
      'alter table "location" drop constraint "location_warehouse_id_foreign";',
    );

    this.addSql(
      'alter table "location" drop constraint "location_code_unique";',
    );
